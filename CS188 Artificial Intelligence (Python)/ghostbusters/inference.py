...
class Particle():
    """container class for particle"""
    def __init__(self, state = (0,0), weight = 1.0):
            self.state = state
            self.weight = weight
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for x in xrange(0,self.numParticles):
        self.particles.append(Particle(random.choice(self.legalPositions),1.0))
  ...
  def observe(self, observation, gameState):
    ...
    if (noisyDistance == None):
        """case if ghost has been captured by Pacman"""
        for particle in self.particles:
            particle.state = self.getJailPosition()
            particle.weight = 1.0
    else:
        allZero = True
        for particle in self.particles:
            particle.weight = emissionModel[util.manhattanDistance(pacmanPosition, particle.state)] 
            if(particle.weight != 0):
                allZero = False
        if(allZero):
            """case if all particles receive 0 weight"""
            self.initializeUniformly(None)
        else:
            samplingDistribution = []
            samplingValues = []
            for particle in self.particles:
                samplingDistribution.append(particle.weight)
                samplingValues.append(particle.state)
            newParticlesList = []
            for x in xrange(0,self.numParticles):
                newParticlesList.append(Particle(util.sample(samplingDistribution,samplingValues),1.0))
  ...
  def getBeliefDistribution(self):
    ...
    for particle in self.particles:
        beliefs[particle.state] += particle.weight
    beliefs.normalize()
    return beliefs
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    self.particles = []
    while numPart != 0:
        particleTuple = [] #actually a list, but we'll convert later
        for ghost in range(self.numGhosts):
            particleTuple.append(random.choice(self.legalPositions))
        self.particles.append(tuple(particleTuple))
        numPart -= 1
  ...
  def elapseTime(self, gameState):
    ...
    """
    #remember each particle is a tuple of ghost positions
    newParticles = []
    for oldParticle in self.particles:
      newParticle = list(oldParticle) # A list of ghost positions
      newParticleBeliefs = util.Counter()
      for ghostIndex in range(self.numGhosts):
          ...
          # the above is a new dist for the ghostIndex'th ghost. now we need to sample that.
          newParticle[ghostIndex] = util.sampleFromCounter(newPosDist)
  ...
  def observeState(self, gameState):
    ...
    for oldParticle in self.particles:
        newParticle = []
        for ghostIndex in xrange(0,self.numGhosts):
            if(noisyDistances[ghostIndex] == None):
                newParticle.append(self.getJailPosition(ghostIndex))
                emissionModels[ghostIndex][util.manhattanDistance(pacmanPosition,self.getJailPosition(ghostIndex))] = 1.0
            else:
                newParticle.append(oldParticle[ghostIndex])
    ...
    for newParticle in newParticles:
        weight = 1
        for ghostIndex in xrange(0,self.numGhosts):
            weight = weight*emissionModels[ghostIndex][util.manhattanDistance(pacmanPosition, newParticle[ghostIndex])]
        sampleDistribution.append(weight)
        if(weight != 0):
            allZero = False
    if(allZero):
        self.initializeParticles()
        ...
        newerParticles = []
        for x in xrange(0,self.numParticles):
            newerParticles.append(tuple(util.sample(sampleDistribution,newParticles)))
        self.particles = newerParticles

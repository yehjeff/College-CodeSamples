...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for distribution in livingGhostPositionDistributions:
        mostLikelyGhostPositions.append(distribution.argMax())
        
    """find the closest ghost position"""
    closestGhostPosition = None
    closestGhostDistance = ()
    for position in mostLikelyGhostPositions:
        distanceFromPacman = self.distancer.getDistance(pacmanPosition, position)
        if (distanceFromPacman < closestGhostDistance):
            closestGhostDistance = distanceFromPacman
            closestGhostPosition = position
    
    """find action that gets you closer to closest ghost position"""
    bestAction = None
    bestFutureDistance = ()
    for action in legal:
        futurePosition = Actions.getSuccessor(pacmanPosition, action)
        futureDistance = self.distancer.getDistance(futurePosition, closestGhostPosition)

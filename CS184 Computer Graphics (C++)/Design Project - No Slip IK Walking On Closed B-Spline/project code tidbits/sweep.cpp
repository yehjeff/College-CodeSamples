    ...
    for (int i = 1; i < size-1; i++) {
        ...
        if (i > 1) {
            ...
            for (int v = 0; v <= csize; v++) {
				...
				glVertexAttrib3dARB(shader.getTangentAttrib(), tan0[VX], tan0[VY], tan0[VZ]);
				glVertexAttrib3dARB(shader.getBitangentAttrib(), bitan0[VX], bitan0[VY], bitan0[VZ]);
                ...
                // @TODO: SET TANGENT AND BITANGENT
                // HINT: Use the glVertexAttrib3fARB, like:
				glVertexAttrib3dARB(shader.getTangentAttrib(), tan1[VX], tan1[VY], tan1[VZ]);
				glVertexAttrib3dARB(shader.getBitangentAttrib(),bitan1[VX], bitan1[VY], bitan1[VZ]);
...
void Sweep::generateSurfaceCoords(vector<vector<vec3>> &surface){
	vector<PathPoint> polyline; 
	vector<vec2> profile; 
	double crossSectionScale=0.3;//change if call to renderWithDisplay list in Main is changed
	int pathSamplesPerPt=2;//change if call to renderWithDisplay list in Main is changed
	int xsectSamplesPerPt=2;//change if call to renderWithDisplay list in Main is changed
	int totalSamples = (int) pathPts.size() * pathSamplesPerPt;
	sampleBSpline(pathPts, polyline, 10*totalSamples);
	sampleBSpline(profilePts, profile, 10*xsectSamplesPerPt * int(profilePts.size()));

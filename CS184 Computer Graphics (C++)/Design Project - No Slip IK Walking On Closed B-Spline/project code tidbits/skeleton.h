...
struct Joint {
    Joint() {}
    Joint(int parent, vec3 posn) : parent(parent), length(0), posn(posn), locked(false) {}
...
struct Animation
{
	...
	vector<vec3> roots;
    void setJoints(vector<Joint> &joints, double frame,int rootIndex);

    // save the orientations from a joint vector as a frame of animation
    void addAsFrame(vector<Joint> &joints,vec3 root) {
...
public:
	...
	void inverseKinematicsLocked(int j,vec3 pos, int mode);
	...
	void calcJacobian(vector<vector<double>> *j,vector<Joint*> chain,vec3 target);
	void vecMatMult(vector<vector<double>> *jacobian, vector<double> *dxyz, vector<double> *paramDisp);
	void applyDisplacement(vector<Joint*> chain, vector<double> *paramDisplacement);
	void calcJacobianLocked(vector<vector<double>> *j,vector<Joint*> chain,vec3 target);
	void applyDisplacementLocked(vector<Joint*> chain, vector<double> *paramDisplacement);

	void moveRoot(vec3 newRootPos);
	void lockJoint(int i){
		joints[i].locked=true;
	}
	void unlockJoint(int i){
